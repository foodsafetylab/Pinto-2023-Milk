...
#Summer time and temperature profiles

Time_Temp_Profile_summer<-read.csv("5-Day-Milk-Data-Summer.csv")

#Milk Refrigerated control data
Time_Temp_Profile_RC_summer<-read.csv("5-Day-Refrigerated-Control-Data-Summer.csv")
```
...
Func_Growth_LagCon<-function(In_Lag_Consumed,Time_Temp_df,Interval, AF){
  ...
  for (i in 1:nrow(Time_Temp_df)){
    if (Total_Lag_Consumed <1 && old_lag!=0){
      ...
      Growth = ((new_growth_rate(newTemp = Time_Temp_df$Rtemp[i], oldMu = old_mumax))/2.303)* AF #0.684 #Converted log10 from log ln
...
An adjustment factor of 1.34 was chosen to match the growth rate obtained to match the experimental trials for this study. 
...
```{r}
#Winter
Output_Milk<-Spoilage_Function_Single_Milk(Cont = 0, Pop_Max =8.14, Time_Temp_df = Time_Temp_Profile, Interval =1/60,AF = 1.34)
...
#Summer
Output_Milk_Summer<-Spoilage_Function_Single_Milk(Cont = 0, Pop_Max =8.14, Time_Temp_df = Time_Temp_Profile_summer, Interval =1/60,AF = 1.34)
Changes_Over_Time_Summer<-Output_Milk_Summer[[2]]
plot(Changes_Over_Time_Summer)
#max(Changes_Over_Time_Summer)
#Changes_Over_Time_Summer[1441]

NMax = 8.14
for (i in 1:nrow(AnalysisDF_Summer)){
  In_Cont<-AnalysisDF_Summer$InSpoilageCon[i]
  Time<-AnalysisDF_Summer$TotTime[i]
  if (Time == 0){
    Total_Growth = 0
  }else{
    Total_Growth<-Changes_Over_Time_Summer[Time]
  }
  Final_Con<-In_Cont+Total_Growth
  if(Final_Con>NMax){
    Final_Con <- NMax
  }
  AnalysisDF_Summer$SpoilageCon[i]<-Final_Con
}
...
AnalysisDF_Summer$Season <-"Summer"
...
ggsave("Figures/Share Table Spoilage-log-summer.jpg", width =8, height = 5, units = "in", dpi = 300)
...
ggsave("Figures/Share Table Spoilage-NonLog-summer.jpg", width =8, height = 5, units = "in", dpi = 300)
...
Table_s3
write.csv(Table_s3,"Table S3.csv")
...
write.csv(TableS4,"Table S4.csv")
...
#Overall
Overall_s3_summer<-AnalysisDF_Summer %>% 
  #filter(Shared == "No") %>% 
  summarize(TimeMedian= median(TotTime), 
            Timeq25 = quantile(TotTime, 0.025), 
            Timeq975 = quantile(TotTime, 0.975),
...
NotShared_s3_summer<-AnalysisDF_Summer %>% 
  filter(Shared == "No") %>% 
  summarize(TimeMedian= median(TotTime), 
            Timeq25 = quantile(TotTime, 0.025), 
            Timeq975 = quantile(TotTime, 0.975),
...
Shared_s3_summer<-AnalysisDF_Summer %>% 
  filter(Shared == "Yes") %>% 
  summarize(TimeMedian= median(TotTime), 
            Timeq25 = quantile(TotTime, 0.025), 
            Timeq975 = quantile(TotTime, 0.975),
...
Table_s3_summer<-rbind(Overall_s3_summer, NotShared_s3_summer, Shared_s3_summer)
Table_s3
write.csv(Table_s3_summer,"Table S3 summer.csv")


### Table S4
TableS4_summer<-AnalysisDF_Summer %>% 
  group_by(Location) %>% 
  summarise(count = n(),
            percentage = count/nrow(.),
            day1= sum(TotTime<1440),
...
write.csv(TableS4_summer,"Table S4 Summer.csv")
